...
The Jensen-Shannon divergence between two probability distributions is non-negative and $JSD(p_{data}||p_g) = 0$ iff $p_{data} = p_g$.

Therefore, in order for $G$ to minimise the value function, the **only** solution is $p_{data} = p_g$. Therefore,

$$
\underset{G}{min}\space V(G, D^*) = -log4
$$
